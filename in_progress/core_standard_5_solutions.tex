...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textbf{Core Assessment 3 2025-10-01}}
\fancyfoot[C]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}
\fancypagestyle{plain}{%
  \fancyhf{}
  \fancyhead[L]{\textbf{Core Assessment 3}}
  \fancyfoot[C]{\thepage}
  \renewcommand{\headrulewidth}{0.4pt}
}
...
\begin{multicols}{3}
\section*{A}
\textbf{Fill in the blanks.} Each item below asks you to supply the missing value—either \(a\), \(b\), \(a \bmod b\), or \(a \,\mathrm{div}\, b\). If no valid number exists for the blank, “not possible” is an acceptable answer.
...
\section*{E}
\section*{Base Modular Rules}

\begin{enumerate}
\item[\textbf{R1}] \textbf{Definition:} 
\(a \equiv b \pmod m \iff m \mid (a - b)\).

\item[\textbf{R2}] \textbf{Ops:} If \(a \equiv b\), \(c \equiv d\) (mod \(m\)):
\(a\pm c \equiv b\pm d\),
\(ac \equiv bd\),
\(a^n \equiv b^n\).

\item[\textbf{R3}] \textbf{Reduction:} 
\((a+m)^n \equiv a^n \pmod m\).

\item[\textbf{R4}] \textbf{Parity Sign:} 
\((-a)^n = a^n\) (even \(n\)); 
\((-a)^n = -a^n\) (odd \(n\)).

\item[\textbf{R5}] \textbf{Factorization:} 
\(x^2 - y^2 = (x - y)(x + y)\); 
\(x^3 - y^3 = (x - y)(x^2 + xy + y^2)\).

\item[\textbf{R6}] \textbf{Residues:} 
Check reps \(0,\dots,m-1\) to prove;
one counterexample disproves.

\item[\textbf{R7}] \textbf{Freshman’s Dream:}  
If \(p\) is prime,
\[
...
\textbf{Using the Rules above, prove that the following is true, or provide a counterexample.}


\begin{enumerate}

\item $3\mid(2^{2n}-1)$ for all $n\ge1$.
\begin{solutionbox}
$2 \equiv -1$ \hfill (R1)\\
$2^{2n} \equiv (-1)^{2n}$ \hfill (R2)\\
$(-1)^{2n} = 1$ \hfill (R4)\\
$2^{2n}-1 \equiv 0$ \hfill (R2)
\end{solutionbox}

\item $3\mid(n^3 - n)$ for all $n$.
\begin{solutionbox}
Check $n \equiv 0,1,2$ \hfill (R6)\\
For each residue, $n^3 - n \equiv 0$ \hfill (R2)
\end{solutionbox}

\item $4\mid(n^4 - n)$ for all $n$.
\begin{solutionbox}
Counterexample $n=2$ \hfill (R6)
\end{solutionbox}

\item $3\mid((2+1)^k - 2^k - 1)$ for all $k\ge0$.
\begin{solutionbox}
Counterexample $k=2$ \hfill (R6)
\end{solutionbox}

\item $3\mid((-2)^{2n}+2)$ for odd $n$.
\begin{solutionbox}
$(-2)^{2n} = 2^{2n}$ \hfill (R4)\\
$2 \equiv -1$ \hfill (R1)\\
$2^{2n} \equiv (-1)^{2n}$ \hfill (R2)\\
$(-1)^{2n} = 1$ \hfill (R4)\\
$1+2 \equiv 0$ \hfill (R2)
\end{solutionbox}

\item $3\mid(a^3 - b^3)$ if $3\mid(a-b)$.
\begin{solutionbox}
$a^3 - b^3 = (a - b)(a^2 + ab + b^2)$ \hfill (R5)\\
$3 \mid (a - b)$ \hfill (R1)\\
$3 \mid (a^3 - b^3)$ \hfill (R2)
\end{solutionbox}

\item $5\mid((4+1)^k - 4^k - 1^k)$.
\begin{solutionbox}
Counterexample $k=2$ \hfill (R6)
\end{solutionbox}

\item $3\mid((a+3)^k - a^k)$ for any integer $a$.
\begin{solutionbox}
$(a+3)^k \equiv a^k$ \hfill (R3)\\
$(a+3)^k - a^k \equiv 0$ \hfill (R2)
\end{solutionbox}

\item Show that $3^{10}\equiv 2^{10}$ mod $5$.
\begin{solutionbox}
$3 \equiv -2$ \hfill (R1)\\
$3^{10} \equiv (-2)^{10}$ \hfill (R2)\\
$(-2)^{10} = 2^{10}$ \hfill (R4)
\end{solutionbox}

\item Find all $n$ such that $7^{n}\equiv 3^{n}$ mod $10$.
\begin{solutionbox}
$7 \equiv -3$ \hfill (R1)\\
$7^n \equiv (-3)^n$ \hfill (R2)\\
$(-3)^n = 3^n$ iff $n$ even \hfill (R4)
\end{solutionbox}

\item Show that 
\[
5 \mid ( (1+3+4)^{20} - 1 - 3^{20} - 1 ).
\]
\begin{solutionbox}
$(1+3+4)^{20} \equiv (1+3)^{20} + 4^{20}$ \hfill (R7)\\
$4^{20} \equiv 1$ \hfill (R2)\\
$(1+3)^{20} \equiv 1 + 3^{20}$ \hfill (R7)\\
Subtract terms \hfill (R2)
\end{solutionbox}
